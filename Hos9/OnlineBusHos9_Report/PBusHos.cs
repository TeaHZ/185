...
{
    internal class PBusHos9_Report : ProcessingBusinessAsyncResult
    {
        public override bool ProcessingBusiness(SLBBusinessInfo InBusinessInfo, out SLBBusinessInfo OutBusinessInfo)
        {
            ...
            try
            {
                ...
                switch (name)//CCN.ToString().Substring(CCN.ToString().Length - 4)
                {
                    ...
                    case "0013"://查询医学证明列表 （TYPE：GETMEDCERTIFICATE）
                        OutBusinessInfo.BusData = BUS.GETMEDCERTIFICATE.B_GETMEDCERTIFICATE(InBusinessInfo.BusData);
                        break;
                    case "0014"://查询医学证明明细（TYPE：GETMEDCERTIFICATEDATA）
                        OutBusinessInfo.BusData = BUS.GETMEDCERTIFICATE.B_GETMEDCERTIFICATEDATA(InBusinessInfo.BusData);
                        break;
                    case "0015"://医学证明回传（TYPE：ZZJMEDICALEVIDENCEPRNBACK）
                        OutBusinessInfo.BusData = BUS.GETMEDCERTIFICATE.B_ZZJMEDICALEVIDENCEPRNBACK(InBusinessInfo.BusData);
                        break;
