...
namespace ZZJ_EInvoice
{
    ...
    public abstract class DbHelperMySQL
    {
        ...
        //public static int GetSysId(string sysIdName)
        //{
        //    int sysId = -1;
        //    if (!new GetSysId().GetSysIdBase(sysIdName, out sysId))
        //    {
        //    }
        //    return sysId;
        //}
        ...
        /// 获取指定业务ID
        /// </summary>
        /// <param name="BusID_NAME">标识</param>
        /// <param name="Sys_Id">最新ID</param>
        /// <returns></returns>
        public static bool GetBusIdBase(string BusID_NAME, out int Sys_Id)
        {
            Sys_Id = -1;
            using (MySqlConnection conn = new MySqlConnection(connectionString))
            {
                string sp_Name = @"GETBUSIDBASE";

                if (conn.State != ConnectionState.Open) conn.Open();
                MySqlCommand sqlcmd = new MySqlCommand(sp_Name, conn);
                sqlcmd.CommandType = CommandType.StoredProcedure;

                sqlcmd.Parameters.AddWithValue("SYSIDNAME", BusID_NAME);
        ...
        public static void ExecuteSqlTran(Hashtable SQLStringList, bool MustHaveData = true)
        {
            if (SQLStringList.Count == 0 && MustHaveData)
            {
        ...
        /// </summary>
        /// <param name="SQLStringList">SQL语句的哈希表（key为sql语句，value是该语句的MySqlParameter[]）</param>
        public static void ExecuteSqlTran(Dictionary<string, MySqlParameter[]> SQLStringList)
        {
            ArrayList list = new ArrayList(SQLStringList.Values);
            using (MySqlConnection conn = new MySqlConnection(connectionString))
            {
                conn.Open();
                using (MySqlTransaction trans = conn.BeginTransaction())
